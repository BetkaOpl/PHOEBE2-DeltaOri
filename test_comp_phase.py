...
class Myphoebe2(Myphoebe):
  ...
  def chi2_contrib(self, ids, dataset):
    ...
    return np.sum(self.chi[ids])


  def plot_forward_model_phase_com(self, output, dpi=600):
    '''
    ...
    x, y = zip(*sorted(zip(phase(self.x[ids]), self.ysyn[ids]-down_shift), reverse=False, key=lambda x: x[0]))
    ax1.plot(x, y, color=cfg._sections['colors'].get('syn'), lw=lw, label='BRITE - sythetic', zorder=3)
    ...
    chi2_sum = self.chi2_contrib(ids, 3)
    ...
    x, y = zip(*sorted(zip(phase(self.x[ids]), self.ysyn[ids]), reverse=False, key=lambda x: x[0]))
    ax1.plot(x, y, color=cfg._sections['colors'].get('syn'), lw=lw, zorder=3)
    ...
    chi2_sum += self.chi2_contrib(ids, 4)
    ...
    x, y = zip(*sorted(zip(phase(self.x[ids]), self.ysyn[ids]), reverse=False, key=lambda x: x[0]))
    ax2.plot(x, y, color=cfg._sections['colors'].get('syn'), lw=lw, label=r'RV - sythetic', zorder=3)
    ...
    chi2_sum +=  self.chi2_contrib(ids, 1)
    ids = np.where(self.dataset==2)

    x, y = zip(*sorted(zip(phase(self.x[ids]), self.ysyn[ids]), reverse=False, key=lambda x: x[0]))
    ax2.plot(x, y, color=cfg._sections['colors'].get('syn'), lw=lw, zorder=3)
    ...
    chi2_sum += self.chi2_contrib(ids, 2)
    ...
    print('SUMA chi2 in title: ', chi2_sum)

    title=r'$\chi^2$ = {}'.format(int(round(chi2_sum, 0)))
...
def main():
  ...
  theta = np.loadtxt('best_fit.csv')
  theta = theta[1:]

  myphoebe.chi2(theta)

  myphoebe.plot_forward_model_phase_com('Final_fixT.eps')
