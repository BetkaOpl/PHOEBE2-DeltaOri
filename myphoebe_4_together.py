...
class Myphoebe(object):
  ...
  def __init__(self, debug=True):
    ...
    '''
    self.debug    = debug
    self.T1_fixed = False    # Should be Teff of the primary fixed?
    self.l3_fixed = False    # Should be l3 (B, R) fixed?
  ...
  def model(self, theta):
    ...
    '''
    if self.T1_fixed and self.l3_fixed:
        T0,T2,R1,R2,I,SA,SB,M1,M2,e,omega,gamma = theta

    if (not self.T1_fixed) and self.l3_fixed:
        T0,T1,T2,R1,R2,I,SA,SB,M1,M2,e,omega,gamma = theta

    if self.T1_fixed and (not self.l3_fixed):
        T0,T2,R1,R2,I,SA,SB,M1,M2,e,omega,gamma,l3B,l3R = theta

    if (not self.T1_fixed) and (not self.l3_fixed):
        T0,T1,T2,R1,R2,I,SA,SB,M1,M2,e,omega,gamma,l3B,l3R = theta

    if not self.T1_fixed:
        self.b.set_value('teff',       component='primary',   value=T1*u.K)

    if not self.l3_fixed:
  ...
  def initial_parameters(self):
    ...
    if not self.T1_fixed and self.l3_fixed:
        theta = T0,T1,T2,R1,R2,I,SA,SB,M1,M2,e,omega,gamma

    if self.T1_fixed and (not self.l3_fixed):
        theta = T0,T2,R1,R2,I,SA,SB,M1,M2,e,omega,gamma,l3B,l3R

    if (not self.T1_fixed) and (not self.l3_fixed):
        theta = T0,T1,T2,R1,R2,I,SA,SB,M1,M2,e,omega,gamma,l3B,l3R
  ...
  def lnprior(self, theta):
    ...
    if  2457733.8493-0.2 < T0 < 2457733.8493+0.2 and \
                   ...
                   90    < omega < 180   and \
                   -5    < gamma < 35    and \
                   self.T1_fixed or (25000 < T1  < 35000) and \
                   self.l3_fixed or (0.15  < l3B < 0.4)   and \
                   self.l3_fixed or (0.15  < l3R < 0.4):
        return 0.0
    else:
        return -np.inf
